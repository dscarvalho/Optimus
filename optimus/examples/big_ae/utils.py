...
class TokenDataset(Dataset):
    def __init__(self, tokenizers, args, file_path='train', text_split_mode='natural', block_size=512):
        ...
        # Bert tokenizer special tokens
        self.bert_pad_token=tokenizers[0].convert_tokens_to_ids([tokenizers[0].pad_token])[0]
        # self.bert_sep_token=tokenizers[0].convert_tokens_to_ids([tokenizers[0].pad_token])[0]
    ...
    def _read_corpus_natural_split(self, fname, label, max_length, block_size, args):
        ...
        for p1, p2, c in zip(data['premise1'], data['premise2'], data['conclusion']):
            ...
            # change here add_special_tokens_sentences_pair(token_ids_0, token_ids_1) in tokenization_bert.py
            tokenized_text0_0 = self.tokenizers[0].convert_tokens_to_ids(self.tokenizers[0].tokenize(p1))
            tokenized_text0_1 = self.tokenizers[0].convert_tokens_to_ids(self.tokenizers[0].tokenize(p2))
            tokenized_text0 = self.tokenizers[0].add_special_tokens_sentences_pair(tokenized_text0_0, tokenized_text0_1)

            # show input format

            tokenized_text0_length = len(tokenized_text0)

            tokenized_text1 = self.tokenizers[1].convert_tokens_to_ids(self.tokenizers[1].tokenize(c))
        ...
        #
        #         # change here add_special_tokens_sentences_pair(token_ids_0, token_ids_1) in tokenization_bert.py
        #         # tokenized_text0_0 = self.tokenizers[0].convert_tokens_to_ids(self.tokenizers[0].tokenize(split_line_text_0))
        #         # tokenized_text0_1 = self.tokenizers[0].convert_tokens_to_ids(self.tokenizers[0].tokenize(split_line_text_1))
        #         # tokenized_text0 = self.tokenizers[0].add_special_tokens_sentences_pair(tokenized_text0_0, tokenized_text0_1)
        #
        #         tokenized_text0_length = len(tokenized_text0)
        #
