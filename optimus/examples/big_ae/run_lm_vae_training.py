...
# fine-tune: 1. fixed pre-trained LM just train the last layer. 2. train LM and last layer at the same time. 3. train LM with small lr and train last layer with big lr.
...
def train(args, train_dataloader, model_vae, encoder_tokenizer, decoder_tokenizer, table_name):
    ...
    loss_stat = pd.DataFrame(columns=['iter', 'loss', 'kl_loss'])
    model_vae.zero_grad()
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #     print(encoder_tokenizer.convert_ids_to_tokens(t.numpy()))
            # exit()
            ...
            # print(decoder_tokenizer.convert_ids_to_tokens(labels[2].numpy()))
            ...
            # if model_vae.module:
            #     # model_vae.module not work in cpu
            ...
            loss_stat = loss_stat.append({'iter': step + epoch*len(epoch_iterator), 'loss': loss.item(), 'kl_loss': loss_kl.item()}, ignore_index=True)
    ...
    return global_step, tr_loss / global_step, optimizer, loss_stat
...
def main():
    ...
    if args.use_pretrained_model:
        ...
        encoder_config_class, encoder_model_class, encoder_tokenizer_class = MODEL_CLASSES[args.encoder_model_type]
        model_encoder = encoder_model_class.from_pretrained(output_encoder_dir, latent_size=args.latent_size)
        # tokenizer bert-base-cased 'bert-large-uncased-whole-word-masking-finetuned-squad'
        tokenizer_encoder = encoder_tokenizer_class.from_pretrained(args.encoder_tokenizer_name if args.encoder_tokenizer_name else args.encoder_model_name_or_path, do_lower_case=args.do_lower_case)
        ...
        # Load full model
        checkpoint = torch.load(os.path.join(output_full_dir, 'training.bin'), map_location='cpu') # notice use it map_location='cpu' if GPU is not access.
    ...
    # print('####################')
    # for param_tensor in model_vae.state_dict():
    #     print(param_tensor, "\t", model_vae.state_dict()[param_tensor].size())
    #
    # exit()
    ...
    if args.do_train:
        ...
        # encoder:[batch, max_len], decoder:[batch, max_len], [actual_len, actual_len]
        ...
        global_step, tr_loss, optimizer, loss_stat = train(args, train_dataloader, model_vae, tokenizer_encoder, tokenizer_decoder, table_name)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
        loss_stat.to_csv('loss_stat.csv')
